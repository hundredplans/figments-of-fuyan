...
func onEnemyInVisionMovement(Unit: UnitGD) -> void:
	pass
	
func onNoneInVisionMovement(Unit: UnitGD) -> void:
	onRandomMovement(Unit)
	
func onEnemyInVisionDiscovered(Unit: UnitGD, movement_path: MovementPathGD) -> void:
	pass # stitch together with onEnemyInVision movement

func onBeginUnitMovement(Unit: UnitGD) -> void:
	onNoneInVisionMovement(Unit)
	#if Unit.getVisibleEnemies(): onEnemyInVisionMovement(Unit)
	#else: onNoneInVisionMovement(Unit)
	...
	#var movement_paths: Array = Tiles.onCreateMovementPaths(Unit)
	#
	##onCreateMovementPathInEnemyVision()
	#onCreateMovementPathOutOfEnemyVision()
	
	#while(movements <= Unit.max_speed and onUseNextMovementSpeed(movement_index)):
		#if in_enemy_vision or !(Unit.getVisibleEnemies().is_empty()): # movement in vision
			#in_enemy_vision = true
			## Implement this later for now focus out of vision
			#continue
			
		#onRandomMovement(Unit, movement_paths, movement_index)
		#onExploreMovement(Unit)
	...
	# if at end of movement adjacent to unit attacks it?
	#Unit.Tile = Tile
...
func onExploreMovement(Unit: UnitGD) -> void: pass
...
func onRemoveInvalidDistanceRandomMovementPaths(Unit: UnitGD, speed: int) -> Array:
	var _movement_paths: Array = Tiles.onCreateMovementPaths(Unit, speed)
...
#func onChosenPathSelected(Unit: UnitGD, chosen_path: Dictionary) -> void:
	...
	#for i in range(chosen_path.size):
		...
		#else: Units.onAttackEnemy(Unit, chosen_path.tiles[i])
