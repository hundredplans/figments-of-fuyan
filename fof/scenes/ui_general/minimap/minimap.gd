...
func on_load_minimap(_GameState) -> void:
	...
	var map_size: int = GameState.save_info.map_info.map_size
	
	var sprite_offset: int = (size.y - (10 * 2)) / (map_size + 1)
	for node_info in GameState.save_info.map_info.nodes + [[0, 1, map_size]]:
		var node_type: int = -1 if Vector2(node_info[1], node_info[2]) == GameState.save_info.map_progress else node_info[0]
		...
		if node_info[2] == map_size - 1:
			new_arrows.append([[1, map_size], [node_info[1], node_info[2]]])
		
	for arrow_info in GameState.save_info.map_info.arrows + new_arrows:
		var Arrow := Line2D.new()
		Arrow.default_color = Helper.RED if Vector2(arrow_info[0][0], arrow_info[0][1]) == GameState.save_info.map_progress else Color("000000")
