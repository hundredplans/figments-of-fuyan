...
func onCompileUnit(file: String) -> void:
	...
	if glb.get_child(0).has_node("Skeleton3D"):
		var scene_path: String = glb.scene_file_path.left(-4) + ".tscn"
		if !FileAccess.file_exists(scene_path):
			...
			var SceneTileObject: Node3D = load(scene_path).instantiate()
			add_child(SceneTileObject)
			glb.collision_points = SceneTileObject.collision_points.duplicate()
			
			for child in SceneTileObject.get_child(0).get_node("Skeleton3D").get_children():
				if child is StaticBody3D:
					child.reparent(glb.get_child(0).get_node("Skeleton3D"))
					var col: CollisionShape3D = child.get_child(0)
					col.owner = glb
					child.owner = glb
					break
			SceneTileObject.queue_free()
