...
func _ready():
	for child in $LobbyAreas.get_children():
		...
		child.mouse_exited.connect(func(): if !lobby_current_item_selected and !lobby_current_camera_travel_item_selected: lobby_can_select_item = 0)

func _process(delta: float) -> void:
	
#	print(lobby_can_select_item, "can_select")
#	print(lobby_current_camera_travel_item_selected, "is_travelling")
#	print(lobby_current_item_selected, "is_selected")
		...
		if moving_lobby_camera_initial_position.distance_to(camera.position) > moving_lobby_camera_initial_position.distance_to(path_point_array[0][0]):
			...
			if path_point_array.size():
				...
				if at_item:
					...
					lobby_can_select_item = lobby_current_camera_travel_item_selected
