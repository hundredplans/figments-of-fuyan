...
func getCombatTiles(_Card: CardGD, tiles: Array, enemies: Array, allies: Array) -> Dictionary:
	...
	if allies.size() + 1 >= enemies.size() + OUTNUMBER_ADVANTAGE: # Turn into steppers
...
func getMinAttackDistanceFromInAttackableRange(Tile: TileGD, enemies: Array) -> float:
	return float(enemies.map(func(x: CardGD): return max(Game.getCoordsDistance(Tile.getCoords(), x.getCoords()) - (x.speed + x.getAttackRange()), 0)).min())
