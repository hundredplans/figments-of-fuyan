...
func getOutOfCombatTiles(Card: CardGD, tiles: Array, _allies: Array, _enemies: Array) -> Dictionary:
	...
	for Tile in tiles:
		var total_distance: float = float(enemy_tiles.map(func(x: TileGD): return Game.getCoordsDistance(x.getCoords(), Tile.getCoords())).reduce(sum, 0))
		tiles_by_value[Tile] = total_distance
		
		if total_distance > max_distance:
			max_distance = total_distance
		
	for Tile: TileGD in tiles_by_value:
