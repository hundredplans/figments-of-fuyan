...
@export_range(0, 1, 0.005) var common: float
@export_range(0, 1, 0.005) var rare: float
@export_range(0, 1, 0.005) var exalt: float
@export_range(0, 1, 0.005) var miniboss: float
@export_range(0, 1, 0.005) var boss: float
...
func getDictionary() -> Dictionary:
	...
	assert(dict.values().reduce((func(accum: float, number: float): return accum + number), 0) >= 1)
	return dict
