...
func onLoadTilesCoords(parent: Node3D, tile_info: TileInfoGD) -> void:
	...
	for tile_coords in info.tile_coords[variation]:
		var Tile := TileGD.new()
		Tile.onLoad(SavedDataTile.new(tile_info.id, 0, tile_coords), parent)
...
func onSave() -> SavedDataGameObject:
	return SavedDataObject.new(info.id, variation, coords, tile_rotation, map_rotation, map_position, height)
