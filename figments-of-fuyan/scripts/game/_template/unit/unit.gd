...
func getArea() -> AreaInfoGD:
	for area_info in Helper.getResourcesRecursive(AreaInfoGD):
		if info in area_info.units: return area_info
	return null
	
func getStatHeightPosition() -> Vector3:
	return Vector3(position.x, getHeightInfo().stat_height, position.z)
#endregion
...
func onLoad(data: SavedData, parent: Node3D) -> void:
	super(data, parent)
	team = data.team
	if info.collision_shape != null and parent != null:
		Model = get_child(0)
