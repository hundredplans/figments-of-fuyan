...
#region Exports
@export var id: int
@export var data: Array[MapNodeOddsInfo]
...
#region Globals
static var INFO_PATH: String = "res://resources/game/worlds/"
#endregion

#region Tool
func _init() -> void:
	id = StaticHelper.onAutoIncrementID(WorldInfo, id)
#endregion

#region Map Nodes
func onGenerateBaseMapNodes(parent: Node3D, base_map_location: MapLocation, info: ChampionUnitInfoGD) -> void:
	...
	var unique_node_id: Array[int] = info.unique_node_id if info != null else []
	setEmptySpotsIDS(empty_spots, unique_node_id, map_node_odds)
	setEliteChiefFights(empty_spots, map_node_odds)
...
func generateMapNodeOddsRollable(map_node_odds: Dictionary) -> Dictionary:
	...
	for key in map_node_odds:
		map_node_odds_rollable[key] = {"fight": map_node_odds.regular_fight, "encounter": map_node_odds.encounter, "shop": map_node_odds.shop}
...
#region Set Empty ID
func setEmptySpotsIDS(empty_spots: Array, unique_node_ids: Array[int], map_node_odds: Dictionary) -> void:
	empty_spots.shuffle()
	var guarantee_shop: bool = true
...
#region Elites
func setEliteChiefFights(empty_spots: Array, map_node_odds: Dictionary) -> void:
...
#region Map Nodes
func onLoadMapNodes(empty_spots: Array, parent: Node, base_location: MapLocation) -> void:
	var empty_spot_to_map_node: Dictionary = {}
	for empty_spot in empty_spots:
		var map_location := MapLocation.new(empty_spot.progress, empty_spot.lane, base_location.world, base_location.area, false)
		var saved_data := SavedDataMapNode.new(empty_spot.id, map_location, [])
		var map_node: MapNode = saved_data.onLoad(parent)
		empty_spot_to_map_node[empty_spot] = map_node
		
	for empty_spot in empty_spot_to_map_node:
		var map_node: MapNode = empty_spot_to_map_node[empty_spot]
		for link in empty_spot:
			pass
